...
- Library: Also takes up the scope and small log. Paginated song table allows selecting from hardcoded songs. You may just 'sample' within the menu or over-write the buffer.
...
- Play if we select a song, Pause & use buffer if we're just sampling and exit library
